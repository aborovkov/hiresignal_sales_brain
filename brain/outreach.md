...
# Провизорное решение 2026-08-07 (Alexey): борд-ссылка идёт в ПЕРВОЕ касание, по умолчанию PHP-борд

Отменяет на время правило "демо-ассеты только после ответа". Алексей: ссылка работает,
ставим её в открывашки по умолчанию, дальше смотрим на результат. Помечено как гипотеза,
а не как новое постоянное правило - вернуться к нему, когда наберётся статистика.

**Дефолтный ассет:**

```
https://app.stepup.services/b/OYd6n1s7C8HAf5fAAkzXwlKiy3w6KhNRH04ON7xQ
```

Senior PHP Developer, eCarsTrade, 5 оценённых кандидатов (PHP / Laravel / Docker / Jenkins).
Проверен 2026-08-07: открывается чисто.

## Правило форматирования (родилось из реального сбоя)

**URL идёт отдельной строкой, с пустой строкой после. К хвосту URL ничего не приклеивается.**

2026-08-06 в треде с фракциональным рекрутером (Тбилиси) следующее предложение втянулось
внутрь якоря, между ними встали служебные символы, а слово "Would" стало вторым
гиперлинком. Получатель увидел длинный незнакомый URL с приклеенным текстом и лишней
ссылкой - визуальную подпись фишинга - и ответил "due to security I will not open any
links". См. `objections.md`, полевое возражение 2026-08-06.

## Что проверить, прежде чем делать выводы о гипотезе

Борд открывается у Алексея залогиненным и показывает плашку "Your workspace is open -
you are signed in". **Холодный получатель видит другое.** Если публичный вид просит
входа или ломается, гипотеза провалится молча и это спишут на оффер, а не на страницу.
Открыть в инкогнито и посмотреть глазами получателя.

## Что НЕ отменено

Всё остальное в этом файле в силе: до ~500 знаков, один вопрос и один CTA на сообщение,
без цен, без обещания "отсеем всех неподходящих" (только одобренная трёхуровневая форма),
без длинного тире, без восклицательных знаков в первом касании, никакой выдуманной
персонализации. Каденция и анти-автоматизационные гардрейлы от 2026-08-02 тоже без изменений.

## Первая партия по этому решению

2026-08-07 отправлено 11 первых касаний с этой ссылкой: 7 `agency-tech` и 4
`independent-fractional`, у каждого свой угол и свой текст. FU1 не раньше 2026-08-10.
Это и есть выборка, по которой гипотезу стоит судить.
